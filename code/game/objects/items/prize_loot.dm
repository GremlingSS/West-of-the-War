...
/obj/item/locked_box/examine(mob/user)
	...
	if(user.client.prefs.special_p >= 8)//SPECIAL Integration
		if(trapped)
...
/obj/item/locked_box/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/screwdriver))
		...
		//SPECIAL Integration
		var/success_after_skill = min((user.client.prefs.special_l * 5) + (user.client.prefs.special_p * 5) + success_after_tier, 100) //the higher the persons luck, the better, up to a max of 100, with 90 added
