...
/proc/voice_of_god(message, mob/living/user, list/span_list, base_multiplier = 1, include_speaker = FALSE, message_admins = TRUE)
	...
	//MOVE
		...
		for(var/iter in 1 to 5 * power_multiplier)
			for(var/V in listeners)
				var/mob/living/L = V
				addtimer(CALLBACK(L, GLOBAL_PROC_REF(_step), L, direction? direction : pick(GLOB.cardinals)), 10 * (iter - 1))
...
/proc/velvetspeech(message, mob/living/user, base_multiplier = 1, message_admins = FALSE, debug = FALSE)
	...
	if(findtext(message, enthrall_words))
		for(var/V in listeners)
			...
			if(E.lewd)
				addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='nicegreen'><i><b>[E.enthrallGender] is so nice to listen to.</b></i></span>"), 5)
			E.cooldown += 1
	...
	if(findtext(message, reward_words))
		for(var/V in listeners)
			...
			if(L == user)
				continue
			if (E.lewd)
				addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='love'>[E.enthrallGender] has praised me!!</span>"), 5)
				if(HAS_TRAIT(L, TRAIT_MASO))
					E.enthrallTally -= power_multiplier
					E.resistanceTally += power_multiplier
					E.cooldown += 1
			else
				addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='nicegreen'><b><i>I've been praised for doing a good job!</b></i></span>"), 5)
			E.resistanceTally -= power_multiplier
			E.enthrallTally += power_multiplier
		...
		for(var/V in listeners)
			...
			if (E.lewd)
				if(HAS_TRAIT(L, TRAIT_MASO))
					...
					E.enthrallTally += power_multiplier
					E.resistanceTally -= power_multiplier
					addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='love'>I've let [E.enthrallGender] down...!</b></span>"), 5)
				else
					addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='warning'>I've let [E.enthrallGender] down...</b></span>"), 5)
			else
				addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='warning'>I've failed [E.master]...</b></span>"), 5)
				E.resistanceTally += power_multiplier
				E.enthrallTally += power_multiplier
				E.cooldown += 1
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(0)
					...
					user.emote("snap")
					if(E.lewd)
						addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='big warning'>The snapping of your [E.enthrallGender]'s fingers brings you back to your enthralled state, obedient and ready to serve.</b></span>"), 5)
					else
						addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='big warning'>The snapping of [E.master]'s fingers brings you back to being under their influence.</b></span>"), 5)
	...
	//Antiresist
		...
		for(var/V in listeners)
			...
			var/datum/status_effect/chem/enthrall/E = L.has_status_effect(/datum/status_effect/chem/enthrall)
			E.status = "Antiresist"
			addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='big warning'>Your mind clouds over, as you find yourself unable to resist!</b></span>"), 5)
	...
	//ATTRACT
		...
		for(var/V in listeners)
			...
			L.throw_at(get_step_towards(user,L), 3 * power_multiplier, 1 * power_multiplier)
			E.cooldown += 3
			addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You are drawn towards [user]!</b></span>"), 5)
			...
			switch(E.phase)
				if(2 to INFINITY)
					if(L.m_intent != MOVE_INTENT_WALK)
						L.toggle_move_intent()
						E.cooldown += 1
						addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You slow down to a walk.</b></span>"), 5)
			...
			switch(E.phase)
				if(2 to INFINITY)
					if(L.m_intent != MOVE_INTENT_RUN)
						L.toggle_move_intent()
						E.cooldown += 1
						addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You speed up into a jog!</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(2 to INFINITY)
					L.lay_down()
					E.cooldown += 10
					addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "[(E.lewd?"<span class='love'>You eagerly lie down!":"<span class='notice'>You suddenly lie down!")]</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(2 to INFINITY)
					...
					E.cooldown += 8
					addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You suddenly drop to the ground!</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3)//Tier 3 only
					...
					E.cooldown += 5
					addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You begin to lick your wounds.</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3 to INFINITY)
					...
					E.cooldown += 8
					addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>Your muscles freeze up!</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3 to INFINITY)
					...
					addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You feel your metabolism speed up!</b></span>"), 5)
	...
	//COLD
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3 to INFINITY)
					L.adjust_bodytemperature(-50 * power_multiplier)
					addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You feel your metabolism slow down!</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3 to INFINITY)//Tier 3 only
					...
					addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You jump to your feet from sheer willpower!</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3)//Tier 3 only
					E.status = "pacify"
					E.cooldown += 10
					addtimer(CALLBACK(L, GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You feel like never hurting anyone ever again.</b></span>"), 5)
