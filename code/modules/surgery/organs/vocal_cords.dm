...
/proc/velvetspeech(message, mob/living/user, base_multiplier = 1, message_admins = FALSE, debug = FALSE)
	...
	if(findtext(message, enthrall_words))
		for(var/V in listeners)
			...
			if(E.lewd)
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='nicegreen'><i><b>[E.enthrallGender] is so nice to listen to.</b></i></span>"), 5)
			E.cooldown += 1
	...
	if(findtext(message, reward_words))
		for(var/V in listeners)
			...
			if(L == user)
				continue
			if (E.lewd)
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='love'>[E.enthrallGender] has praised me!!</span>"), 5)
				if(HAS_TRAIT(L, TRAIT_MASO))
					E.enthrallTally -= power_multiplier
					E.resistanceTally += power_multiplier
					E.cooldown += 1
			else
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='nicegreen'><b><i>I've been praised for doing a good job!</b></i></span>"), 5)
			E.resistanceTally -= power_multiplier
			E.enthrallTally += power_multiplier
		...
		for(var/V in listeners)
			...
			if (E.lewd)
				if(HAS_TRAIT(L, TRAIT_MASO))
					...
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='warning'>I've let [E.enthrallGender] down...</b></span>"), 5)
			else
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='warning'>I've failed [E.master]...</b></span>"), 5)
				E.resistanceTally += power_multiplier
				E.enthrallTally += power_multiplier
				E.cooldown += 1
	...
	//FORGET (A way to cancel the process)
		...
		for(var/mob/living/carbon/C in listeners)
			var/datum/status_effect/chem/enthrall/E = C.has_status_effect(/datum/status_effect/chem/enthrall)
			if(E.phase == 4)
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), C, "<span class='warning'>You're unable to forget about [(E.lewd?"the dominating presence of [E.enthrallGender]":"[E.master]")]!</b></span>"), 5)
				continue
			addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), C, "<span class='warning'>You wake up, forgetting everything that just happened. You must've dozed off..? How embarassing!</b></span>"), 5)
			C.Sleeping(50)
			switch(E.phase)
				if(1 to 2)
					E.phase = -1
	...
	//STATE TRIGGERS
		...
		for(var/V in listeners)
			...
			if (E.phase == 3)
				...
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), C, "<span class='notice'>You whisper your triggers to [(E.lewd?"Your [E.enthrallGender]":"[E.master]")].</span>"), 5)
				...
				if(!E.lewd)
					...
					if (E.mental_capacity >= 5)
						...
						if ((findtext(trigger2, custom_words_words)))
							...
							E.mental_capacity -= 5
							addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), H, "<span class='notice'>[(E.lewd?"your [E.enthrallGender]":"[E.master]")] whispers you a new trigger.</span>"), 5)
			...
			if(E.phase == 3)
				if (get_dist(user, H) > 1)//Requires user to be next to their pet.
					...
					if (E.mental_capacity >= 200)
						...
						addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), H, "<span class='notice'>[(E.lewd?"Your [E.enthrallGender]":"[E.master]")] whispers you a new objective.</span>"), 5)
		...
		for(var/V in listeners)
			...
			if(E.phase > 1)
				if(user.ckey == E.enthrallID && user.real_name == E.master.real_name)
					E.master = user
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), H, "<span class='nicegreen'>[(E.lewd?"You hear the words of your [E.enthrallGender] again!! They're back!!":"You recognise the voice of [E.master].")]</b></span>"), 5)
