...
/obj/machinery/pool/controller/proc/scan_things()
	...
	for(var/turf/open/pool/W in cached)
		linked_turfs += W
		W.controller = src
		W.RegisterSignal(src, TYPE_PROC_REF(COMSIG_PARENT_QDELETING, /turf/open/pool,unregister_controller))
	for(var/obj/machinery/pool/drain/pooldrain in cached)
		linked_drain = pooldrain
		linked_drain.controller = src
		linked_drain.RegisterSignal(src, TYPE_PROC_REF(COMSIG_PARENT_QDELETING, /obj/machinery/pool/drain,unregister_controller))
		break
	for(var/obj/machinery/pool/filter/F in cached)
		linked_filter = F
		linked_filter.controller = src
		linked_filter.RegisterSignal(src, TYPE_PROC_REF(COMSIG_PARENT_QDELETING, /obj/machinery/pool/filter,unregister_controller))
		break
