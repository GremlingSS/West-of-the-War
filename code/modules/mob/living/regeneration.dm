// This code was taken directly from Coyote Bayou, credit entirely to Fenny for the initial code.
#define COMSIG_MOB_SLEEPING "mob_sleeping" //from base of /datum/status_effect/incapacitating/sleeping, debuffs.dm
#define BED_HEAL_BONUS 0.03
...
/datum/component/sleeping_regeneration/proc/OnSleep()
	...
	if(buckled_obj && istype(buckled_obj, /obj/structure/bed))
		...
		// We don't define outdoor_areas as a global and this check just makes it so sleep heal bonus doesn't work outside.
		// Given the average player might not be able to piece that without code diving, I commented it out.
		// Rather than define glob.outdoor_areas just for this single proc. -Possum
		//if(!is_type_in_list(get_area(L), GLOB.outdoor_areas))
		//	healAmount += BED_HEAL_BONUS

	// Who even plays a synth?
